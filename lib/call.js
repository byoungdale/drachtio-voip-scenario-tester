...
module.exports = class Call {
  ...
  constructor(user, srf, logger) {
    ...
    this.connect = (req, res) => new Promise((resolve, reject) => {
      ...
      if (!ms) {
        this.logger.debug('Could not create media server');
        return res.send(480);
      } // check for media servers
      this.logger.debug('Created mediaServer');

      ms.connectCaller(req, res, (err, ep, dialog) => {
        if (err) { reject(err); }
        this.logger.debug('created UAC dialog');
    ...
    this.getEndpoint = () => new Promise((resolve, reject) => {
      ...
      this.logger.debug(`Created mediaServer: ${JSON.stringify(ms)}`);
      ...
      ms.createEndpoint({
        ...
        if (err) { reject(err); }
        this.logger.debug(`Created endpoint: ${JSON.stringify(ep)}`);
        resolve(ep);
      }); // ms.createEndpoint
    }); // getEndpoint
    ...
    this.forward = (toContactUri) => {
      return new Promise((resolve, reject) => {
        this.srf.invite((req, res) => {
          this.logger.debug(`forwarding call to ${toContactUri} with a 302 Redirect`);
  ...
  receive(parameters) {
    return new Promise((resolve, reject) => {
      ...
      if (parameters === null) {
        this.srf.invite((req, res) => {
          this.logger.debug('received an INVITE');
          this.connect(req, res)
            .then((ep, dialog) => {
              this.logger.debug('created UAC dialog in received call');
      ...
      setTimeout(() => {
        this.getEndpoint(this.user).then((ep) => {
          this.srf.createUacDialog(`sip:${digits}@${this.user.domain}`, {
            ...
            if (err) { reject(err); }
            this.logger.debug(`created UAC dialog on sent call: ${JSON.stringify(dialog)}`);
            const incall = new InCall(ep, dialog, this.logger);
            resolve(incall);
          }); // srf.createUacDialog
        }); // getEndpoint
